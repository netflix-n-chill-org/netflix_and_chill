package com.example.case_study_03.repository;

import com.example.case_study_03.Controller.ConnectionConfig;
...
import static com.example.case_study_03.Controller.ConnectionConfig.getConnection;

public class MovieRepository implements IUtilityMovieDao {
    ...
    private static final String DELETE_MOVIE_SQL = "UPDATE movie set status = 0 where id = ?;";
    private static final String SHOW_ALL_MOVIE_SQL = "SELECT id, title, genre, releaseYear, rating, img, english_title FROM movie";
    private static final String SEARCH_FOR_MOVIE_SQL = "SELECT title, genre, releaseYear, rating, english_title FROM movie " +
            "WHERE title LIKE ? OR genre LIKE ? OR releaseYear LIKE ? OR rating LIKE ? OR english_title ?;";
    ...
    public List<Movie> selectAllMovie() throws SQLException {
        List<Movie> movieList = new ArrayList<>();
        ...
        try (Connection connection = getConnection();
            ...
            while (resultSet.next()) {
                ...
                float rating = resultSet.getFloat("rating");
                String englishTitle = resultSet.getString("english_title");
                movieList.add(new Movie(id,title, genre, releaseYear, rating, englishTitle));
            }
        } catch (SQLException e) {
            printSQLException(e);
        }
